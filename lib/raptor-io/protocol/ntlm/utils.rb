...
require 'raptor-io/protocol/ntlm/exceptions'
...
class Utils
  ...
  CONST = RaptorIO::Protocol::NTLM::Constants
  CRYPT = RaptorIO::Protocol::NTLM::Crypt
  XCEPT = RaptorIO::Protocol::NTLM::Exceptions
  ...
  def self.make_ntlmssp_blob_auth(domain, name, user, lm, ntlm, enc_session_key, flags = 0x080201)
    ...
    domain_uni = Net::NTLM::EncodeUtil.encode_utf16le(domain)
    user_uni   = Net::NTLM::EncodeUtil.encode_utf16le(user)
    name_uni   = Net::NTLM::EncodeUtil.encode_utf16le(name)
  ...
  def self.make_ntlm_flags(opt = {})
    ...
    ntlmssp_flags &= 0xfff7_ffff if usentlm2_session && (not use_ntlmv2) && (not send_ntlm)
  ...
  def self.make_ntlmv2_clientchallenge(win_domain, win_name, dns_domain, dns_name,
    ...
    client_challenge ||= RaptorIO::Support::Text.rand_text(8)
    ...
    win_domain = Net::NTLM::EncodeUtil.encode_utf16le(win_domain)
    win_name = Net::NTLM::EncodeUtil.encode_utf16le(win_name)
    dns_domain = Net::NTLM::EncodeUtil.encode_utf16le(dns_domain)
    dns_name = Net::NTLM::EncodeUtil.encode_utf16le(dns_name)
    # Make the AV_PAIRs
    addr_list  = ''
    addr_list  << [2, win_domain.length].pack('vv') + win_domain
    addr_list  << [1, win_name.length].pack('vv') + win_name
    ...
    if spnopt[:use_spn]
      spn= Net::NTLM::EncodeUtil.encode_utf16le("cifs/#{spnopt[:name] || 'unknow'}")
    ...
    #    RaptorIO::Support::Text.rand_text(32)  # MachineId generated on startup on win7 and above
  ...
  def self.create_session_key(
        ntlmssp_flags,
        server_ntlmssp_flags,
        user,
        pass,
        domain,
        challenge_key,
    ...
    if key_exchange
      signing_key = RaptorIO::Support::Text.rand_text(16)
